...
var photoBooth = function () {
  ...
  api.getTranslation = function (key) {
    var translation = i18n(key, config.ui.language);
    var fallbackTranslation = i18n(key, 'en');
  ...
  api.startVideo = function (mode) {
    ...
    if (config.preview.mode === 'gphoto') {
      ...
      var _getMedia = navigator.mediaDevices.getUserMedia || navigator.mediaDevices.webkitGetUserMedia || navigator.mediaDevices.mozGetUserMedia || false;
      ...
      _getMedia.call(navigator.mediaDevices, webcamConstraints).then(function (stream) {
        if (mode === 'preview') {
          $('#video--preview').show();
  ...
  api.thrill = function (photoStyle) {
    ...
    api.startCountdown(nextCollageNumber ? config.collage.cntdwn_time : config.picture.cntdwn_time, $('#counter'), function () {
  ...
  api.cheese = function (photoStyle) {
    ...
    if (config.picture.no_cheese) {
      ...
      var cheesemsg = api.getTranslation('cheese');
      $('.cheese').text(cheesemsg);
    } else {
      var _cheesemsg = api.getTranslation('cheeseCollage');

      $('.cheese').text(_cheesemsg);
      $('<p>').text("".concat(nextCollageNumber + 1, " / ").concat(config.collage.limit)).appendTo('.cheese');
    }
  ...
  api.callTakePicApi = function (data) {
    jQuery.post('api/takePic.php', data).done(function (result) {
      ...
      if (result.error) {
        ...
        $('.loading').empty();
        $('#video--sensor').hide();
        var imageUrl = config.foldersRoot.tmp + '/' + result.collage_file;
        ...
        if (config.collage.continuous) {
          ...
          if (result.current + 1 < result.limit) {
            $('<a class="btn rotaryfocus" href="#">' + api.getTranslation('nextPhoto') + '</a>').appendTo('.loading').click(function (ev) {
              ev.stopPropagation();
              ev.preventDefault();
              $('.loaderImage').css('background-image', 'none');
              imageUrl = '';
            ...
            $('<a class="btn rotaryfocus" href="#">' + api.getTranslation('processPhoto') + '</a>').appendTo('.loading').click(function (ev) {
              ev.stopPropagation();
              ev.preventDefault();
              $('.loaderImage').css('background-image', 'none');
              imageUrl = '';
          ...
          $('<a class="btn rotaryfocus" style="margin-left:2px" href="#">' + api.getTranslation('retakePhoto') + '</a>').appendTo('.loading').click(function (ev) {
            ev.stopPropagation();
            ev.preventDefault();
            $('.loaderImage').css('background-image', 'none');
            imageUrl = '';
          ...
          var abortmsg = api.getTranslation('abort');
          $('.loading').append($('<a class="btn rotaryfocus" style="margin-left:2px" href="#">').text(abortmsg)).click(function () {
            location.assign('./');
          });
  ...
  api.errorPic = function (data) {
    setTimeout(function () {
      ...
      var errormsg = api.getTranslation('error');
      ...
      if (config.dev.reload_on_error) {
        var reloadmsg = api.getTranslation('auto_reload');
        $('.loading').append($('<p>').text(reloadmsg));
        setTimeout(function () {
          api.reloadPage();
        }, 5000);
      } else {
        var _reloadmsg = api.getTranslation('reload');
  ...
  api.processPic = function (photoStyle, result) {
    var tempImageUrl = config.foldersRoot.tmp + '/' + result.file;
    $('.spinner').show();
    $('.loading').text(photoStyle === 'photo' || photoStyle === 'chroma' ? api.getTranslation('busy') : api.getTranslation('busyCollage'));
    ...
    $.ajax({
      ...
      success: function success(data) {
		...
		if (photoStyle === 'collage') {
		       ...
		       request.open("GET","http://192.168.1.224/COLLAGE");
		       request.send();
	       }else{
		       console.log('PIC LED');
		       var request = new XMLHttpRequest();
		       request.open("GET","http://192.168.1.224/PIC");
		       request.send();
	       };
  ...
  api.renderChroma = function (filename) {
    ...
    var imageUrl = config.live_keying.show_all ? config.foldersRoot.images + '/' + filename : config.foldersRoot.keying + '/' + filename;
    var preloadImage = new Image();

    preloadImage.onload = function () {
      $('body').attr('data-main-image', filename);
      console.log(config.foldersRoot.keying + '/' + filename);
      var chromaimage = config.foldersRoot.keying + '/' + filename;
      loader.hide();
  ...
  api.renderPic = function (filename) {
    ...
    $('<img src="api/qrcode.php?filename=' + filename + '"/>').on('load', function () {
      ...
      $(this).appendTo(body);
      $('<p>').css('max-width', this.width + 'px').html(api.getTranslation('qrHelp') + '</br><b>' + config.webserver.ssid + '</b>').appendTo(body);
    }); // Add Print Link
    ...
    resultPage.find('.deletebtn').off('click').on('click', function (ev) {
      ev.preventDefault();
      var msg = api.getTranslation('really_delete_image');
      var really = config["delete"].no_request ? true : confirm(filename + ' ' + msg);
    ...
    api.addImage(filename);
    var imageUrl = config.foldersRoot.images + '/' + filename;
    var preloadImage = new Image();
  ...
  api.startCountdown = function (start, element, cb) {
    var count = 0;
    var current = start;
    var stop = start > 2 ? start - 2 : start;
	...
	console.log(current);
       ...
       if (current == 5) {
               console.log('five');
               var request = new XMLHttpRequest();
               request.open("GET","http://192.168.1.224/BACK");
               request.send();
       }
       if (current == 2) {
               console.log('two');
               var request = new XMLHttpRequest();
               request.open("GET","http://192.168.1.224/TWO");
               request.send();
       }
        ...
        $.ajax({
          ...
          success: function success(data) {
            ...
            if (data.error) {
              ...
              $('#print_mesg').empty();
              $('#print_mesg').html('<div class="modal__body"><span style="color:red">' + data.error + '</span></div>');
            }
            ...
            setTimeout(function () {
              ...
              if (data.error) {
                $('#print_mesg').empty();
                $('#print_mesg').html('<div class="modal__body"><span>' + api.getTranslation('printing') + '</span></div>');
              }
  ...
  $('#send-mail-form').on('submit', function (e) {
    ...
    $.ajax({
      ...
      success: function success(result) {
        if (result.success) {
          if (result.saved) {
            message.fadeIn().html('<span style="color:green">' + api.getTranslation('mailSaved') + '</span>');
          } else {
            message.fadeIn().html('<span style="color:green">' + api.getTranslation('mailSent') + '</span>');
          }
        } else {
  ...
  $('#result').on('click', function () {
    if (!modal.close('#qrCode')) {//api.showResultInner(true);
    }
